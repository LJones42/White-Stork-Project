...
Q7_nesting_colnames <- select(final_data, ends_with("nesting"))
...
apply(final_data, 2, function(x){ length(which(!is.na(x))) })

```

```{r closed question overview, echo=TRUE, messages=FALSE}

library(dplyr)
# All knowledge questions table
oldnames = c("Q4.1_migrate","Q4.2_wingspan","Q4.3_globallyrare","Q5a_amphibians_diet",
         "Q5b_birdeggs.chicks_diet","Q5c_carrion_diet","Q5d_fish_diet",
         "Q5e_foodwaste_diet","Q5f_fruit_diet","Q5g_inverts_diet","Q5h_reptiles_diet","Q5i_seeds_diet",
         "Q5j_smallmammals_diet","Q5k_vegetation_diet","Q5l_Don.tKnow_diet","Q6a_farmland_habitat",
         "Q6b_grassland_habitat","Q6c_wetlands_habitat","Q6d_woodland_habitat",
          "Q6e_urban_habitat","Q6f_Don.tKnow_habitat","Q7a_chimneys_nesting","Q7b_ground_nesting","Q7c_roofs_nesting",
         "Q7d_telegraphpoles_nesting","Q7e_trees_nesting","Q7f_Don.tKnow_nesting")
newnames = c("Do WS migrate?","Size of wingspan?","Are WS globally rare?","Diet-amphibians",
         "Diet-birdeggs.chicks","Diet-carrion","Diet-fish",
         "Diet-foodwaste","Diet-fruit","Diet-invertebrates","Diet-reptiles","Diet-seeds",
         "Diet-smallmammals","Diet-vegetation","Diet-DontKnow","Habitat-farmland",
         "Habitat-grassland","Habitat-wetlands","Habitat-woodland",
         "Habitat-urban","Habitat-DontKnow","Nests-chimneys","Nests-ground","Nests-roofs",
         "Nests-telegraphpoles","Nests-trees","Nests-DontKnow")
# Create new dataframe and rename variables
knowledge_data <-  final_data %>% 
    select(UniqueID_all,SurveyType, ReleaseSite,
         Q4.1_migrate,Q4.2_wingspan,Q4.3_globallyrare,
         Q5a_amphibians_diet, Q5b_birdeggs.chicks_diet,Q5c_carrion_diet,Q5d_fish_diet,
         Q5e_foodwaste_diet,Q5f_fruit_diet,Q5g_inverts_diet,Q5h_reptiles_diet,Q5i_seeds_diet,
         Q5j_smallmammals_diet,Q5k_vegetation_diet,Q5l_Don.tKnow_diet,Q6a_farmland_habitat,
         Q6b_grassland_habitat,Q6c_wetlands_habitat,Q6d_woodland_habitat,Q6e_urban_habitat,
         Q6f_Don.tKnow_habitat,Q7a_chimneys_nesting,Q7b_ground_nesting,Q7c_roofs_nesting,
         Q7d_telegraphpoles_nesting,Q7e_trees_nesting,Q7f_Don.tKnow_nesting) %>% 
   rename_at(vars(oldnames), ~ newnames)
   

factor_vec <- factor(c("0", "1"))
# Create table as dataframe
diet_table <- knowledge_data %>%
  group_by(SurveyType, ReleaseSite) %>% 
  gather("Diet-amphibians","Diet-birdeggs.chicks","Diet-carrion","Diet-fish",
         "Diet-foodwaste","Diet-fruit","Diet-invertebrates","Diet-reptiles","Diet-seeds",
         "Diet-smallmammals","Diet-vegetation","Diet-DontKnow", key = Question, value = Answer) %>% 
    factor(Gender,levels=c(0,1),labels=c('Incorrect','Correct')) %>% 
    group_by(Question,SurveyType, Answer) %>%
    tally() %>%
    # group_by(SurveyType, Question, Answer) %>%
    mutate(Percentage = round(n/sum(n)*100, 2))
diet_table
# Create HTML table
tab_df(diet_table, use.viewer = TRUE)



# Create table as dataframe
knowledge_table_all <- knowledge_data %>%
  group_by(SurveyType, ReleaseSite) %>% 
  gather("Do WS migrate?","Size of wingspan?","Are WS globally rare?","Diet-amphibians",
         "Diet-birdeggs.chicks","Diet-carrion","Diet-fish",
         "Diet-foodwaste","Diet-fruit","Diet-invertebrates","Diet-reptiles","Diet-seeds",
         "Diet-smallmammals","Diet-vegetation","Diet-DontKnow","Habitat-farmland",
         "Habitat-grassland","Habitat-wetlands","Habitat-woodland",
         "Habitat-urban","Habitat-DontKnow","Nests-chimneys","Nests-ground","Nests-roofs",
         "Nests-telegraphpoles","Nests-trees","Nests-DontKnow", key = Question, value = Answer) %>% 
   group_by(Question,SurveyType, Answer) %>%
    tally() %>%
    # group_by(SurveyType, Question, Answer) %>%
    mutate(Percentage = round(n/sum(n)*100, 2))
knowledge_table_all
# Create HTML table
tab_df(knowledge_table_all, use.viewer = TRUE)


# Make a long dataframe
prop.table(table(knowledge_data$SurveyType, knowledge_data$Q4.1_migrate), margin = 1)

...
pref_comms_summary <- pref_comms %>%
  ...
  dplyr::group_by(SiteLocal, Q10_pref, Answer) %>% 
  na.omit %>% # Apply na.omit
  summarise(n = n()) %>%
  mutate(Percent = (n / sum(n)*100)) %>% 
  filter(., Answer == "1")
...
likert_attitudes <- likert_attitudes %>%
  rename("White storks symbolise the beauty of nature" = ".White.storks.symbolise.the.beauty.of.nature.",
    ...
    "I would find it exciting to see white storks in the wild in England" = ".I.would.find.it.exciting.to.see.white.storks.in.the.wild.in.England." ,
         ...
         "Overall, I support efforts that aim to reintroduce the UK's lost species and restore it's natural systems" = 
           ".Overall..I.support.efforts.that.aim.to.reintroduce.the.UK.s.lost.species.and.restore.its.natural.systems.")
# Create likert object and plot
likert_att <- likert(items=likert_attitudes[,1:15], grouping=likert_attitudes[,17])
# Set likert levels
likert_att_levels <- c("Group","Item","Strongly agree", "Agree", "Neutral",
                  "Disagree", "Strongly disagree")
likert_att$results <- likert_att$results[, likert_att_levels]

# Likert plot
plot(likert_att, type="bar", text.size = 4)
# ggsave("Likert_attitudes_all.png", width = 20, height = 50, units = "cm")
...
Q17_management_summary <- Q17_management %>%
  ...
  dplyr::group_by(County, Management_options, Answer) %>% 
  summarise(n = n()) %>%
...
ggplot(Q17_management_summary, 
  aes(x = reorder(Management_options, Percent), y = Percent, fill = County)) +
    geom_bar(position="stack", stat="identity") +
  ...
  theme_minimal( base_size = 12) + 
  coord_flip()
# ggsave("Q17_management_county.png", width = 20, height = 15, units = "cm")

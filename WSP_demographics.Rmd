...
#                               ifelse(final_data$Occupation  %in% other, "Other","Other")))))))
...
demo_columns <- c("SurveyType", "Age_group_match","Age_short", "Gender", "Education", "Education_short",
                                "Occupation", "Occupation_short","Occupation_short_clean", "Region", "Area_type", "Postcode", "ReleaseSite", "SiteProximity", "SiteLocal")
...
all_occupation_df <- all_demo  %>%
  ...
  summarise(counts = n()) %>%
  mutate(Percentage = round(counts/sum(counts)*100, 2))
all_occupation_df$Occupation_short_clean <- ordered(all_occupation_df$Occupation_short_clean, levels = c("Environment/Nature","Natural resource management","Retired","Unemployed","Other","Prefer not to say"))
...
```{r age bar, echo=FALSE, message=FALSE, warning=FALSE}
age_bar
```
```{r gender bar, echo=FALSE, message=FALSE, warning=FALSE}
gender_bar
```

```{r education bar, echo=FALSE, message=FALSE, warning=FALSE}
education_bar
```

```{r occupation bar, echo=FALSE, message=FALSE, warning=FALSE}
occupation_bar
```
...
library("table1")
all_demo$Education_short <- ordered(all_demo$Education_short, 
            levels = c("University graduate","Secondary school/College",
                       "No formal quals.", "Other"))
all_demo$Occupation_short_clean <- ordered(all_demo$Occupation_short_clean, 
            levels = c("Environment/Nature","Natural resource management",
                       "Retired","Unemployed","Other","Prefer not to say"))
...
# Create demo table
short_demo_table <- table1::table1(~Age_short + Gender + Education_short +
...
simp_demo_table <- table1::table1(~Age_short + Gender + Education_short +
